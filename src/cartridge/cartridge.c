...
void handle_cartridge_memory_write(uint16_t addr, uint8_t val, register_file_t *state) {
  ...
  case MBC2:
	...
	if((val & 0xf) == 0x0a) {
      ...
      if(((addr >> 8) & 0x1) == 0x1) {
	if((val & 0x0f) < rom_banks && (val & 0x0f) < max_rom_banks) {
	  switch_rom_bank(val & 0x0f, state);
	} else {
	  printf("DID NOT SWITCH\n");
	}
      }
  ...
  case MBC5:
	...
	ram_enabled = 0;
      ...
      if((val | (curr_rom_bank & 0x100)) < rom_banks && (val | (curr_rom_bank & 0x100)) < max_rom_banks) {
	switch_rom_bank(val | (curr_rom_bank & 0x100), state);
      } else {
	printf("DID NOT SWITCH\n");
      }
